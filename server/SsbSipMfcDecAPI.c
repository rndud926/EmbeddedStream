...
void *SsbSipMfcDecOpen(void)
{
	...
	if (pCTX == NULL)
	{
		printf("SsbSipMfcDecOpen: malloc failed.\n");
		return NULL;
	}
	...
	if (!mapped_addr)
	{
		printf("SsbSipMfcDecOpen: FIMV5.0 driver address mapping failed\n");
		return NULL;
	}
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecInit(void *openHandle, SSBSIP_MFC_CODEC_TYPE codec_type, int Frameleng)
{
	...
	if (openHandle == NULL)
	{
		printf("SsbSipMfcDecSetConfig: openHandle is NULL\n");
	...
	if ((codec_type != MPEG4_DEC)   &&
		...
		(codec_type != DIVX311_DEC) &&
		(codec_type != DIVX412_DEC) &&
		(codec_type != DIVX502_DEC) &&
		(codec_type != DIVX503_DEC) &&
		(codec_type != XVID_DEC)    &&
		(codec_type != VC1RCV_DEC)  &&
		(codec_type != VC1_DEC))
	{
		printf("SsbSipMfcDecOpen: Undefined codec type.\n");
		return MFC_RET_INVALID_PARAM;
	}
	...
	if ((pCTX->codec_type == MPEG4_DEC)   ||
		(pCTX->codec_type == DIVX311_DEC) ||
		(pCTX->codec_type == DIVX412_DEC) ||
		(pCTX->codec_type == DIVX502_DEC) ||
		(pCTX->codec_type == DIVX503_DEC) ||
	...
	{
		printf("SsbSipMfcDecInit: IOCTL_MFC_DEC_INIT (%d) failed\n", DecArg.ret_code);
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecExe(void *openHandle, int lengthBufFill)
{
	...
	if (openHandle == NULL)
	{
		printf("SsbSipMfcDecExe: openHandle is NULL\n");
		return MFC_RET_INVALID_PARAM;
	}

	if ((lengthBufFill < 0) || (lengthBufFill > MAX_DECODER_INPUT_BUFFER_SIZE))
	{
		printf("SsbSipMfcDecExe: lengthBufFill is invalid. (lengthBufFill=%d)\n", lengthBufFill);
		return MFC_RET_INVALID_PARAM;
	}
	...
	if (DecArg.ret_code != MFC_RET_OK)
	{
		printf("SsbSipMfcDecExe: IOCTL_MFC_DEC_EXE failed(ret : %d)\n", DecArg.ret_code);
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecClose(void *openHandle)
{
	...
	if (openHandle == NULL)
	{
		printf("SsbSipMfcDecClose: openHandle is NULL\n");
		return MFC_RET_INVALID_PARAM;
	}
...
void *SsbSipMfcDecGetInBuf(void *openHandle, void **phyInBuf, int inputBufferSize)
{
	...
	{
		printf("SsbSipMfcDecGetInBuf: inputBufferSize = %d is invalid\n", inputBufferSize);
		return NULL;
	}

	if (openHandle == NULL)
	{
		printf("SsbSipMfcDecGetInBuf: openHandle is NULL\n");
		return NULL;
	}
	...
	if (ret_code < 0)
	{
		printf("SsbSipMfcDecGetInBuf: IOCTL_MFC_GET_IN_BUF failed\n");
		return NULL;
	}
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecSetInBuf(void *openHandle, void *phyInBuf, void *virInBuf, int inputBufferSize)
{
	...
	if (openHandle == NULL)
	{
		printf("SsbSipMfcDecSetInBuf: openHandle is NULL\n");
		return MFC_RET_INVALID_PARAM;
	}
...
SSBSIP_MFC_DEC_OUTBUF_STATUS SsbSipMfcDecGetOutBuf(void *openHandle, SSBSIP_MFC_DEC_OUTPUT_INFO *output_info)
{
	...
	if (openHandle == NULL)
	{
		printf("SsbSipMfcDecGetOutBuf: openHandle is NULL\n");
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecSetConfig(void *openHandle, SSBSIP_MFC_DEC_CONF conf_type, void *value)
{
	...
	if (value == NULL)
	{
		printf("SsbSipMfcDecSetConfig: value is NULL\n");
		return MFC_RET_INVALID_PARAM;
	}
	...
	switch (conf_type)
	{
		...
		default:
			printf("SsbSipMfcDecSetConfig: No such conf_type is supported.\n");
			return MFC_RET_INVALID_PARAM;
	}
	...
	if (DecArg.ret_code != MFC_RET_OK)
	{
		printf("SsbSipMfcDecSetConfig: IOCTL_MFC_SET_CONFIG failed(ret : %d, conf_type: %d)\n", DecArg.ret_code, conf_type);
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecGetConfig(void *openHandle, SSBSIP_MFC_DEC_CONF conf_type, void *value)
{
	...
	if (openHandle == NULL)
	{
		printf("SsbSipMfcDecGetConfig: openHandle is NULL\n");
		return MFC_RET_FAIL;
	}

	if (value == NULL)
	{
		printf("SsbSipMfcDecGetConfig: value is NULL\n");
		return MFC_RET_FAIL;
	}
	...
	switch (conf_type)
	{
		...
		case MFC_DEC_GETCONF_CRC_DATA:
			...
			if (DecArg.ret_code != MFC_RET_OK)
			{
				printf("SsbSipMfcDecGetConfig: IOCTL_MFC_GET_CONFIG failed(ret : %d, conf_type: %d)\n", DecArg.ret_code, conf_type);
		...
		default:
			printf("SsbSipMfcDecGetConfig: No such conf_type is supported.\n");
			return MFC_RET_INVALID_PARAM;
	}
...
void tile_to_linear_4x2(unsigned char *p_linear_addr, unsigned char *p_tiled_addr, unsigned int x_size, unsigned int y_size)
{
	...
	for (i = 0; i < y_size; i = i + 16)
	{
		for (j = 0; j < x_size; j = j + 16)
		{
			...
			for (k = 0; k < 16; k++)
			{
				/* 2009.09.15 by icarus limit check - prohibit segmentation fault */
...
int parser(int start_bit, char *temp_buffer, int file_size)
{
	int i;
	...
	for(i=start_bit; i<file_size; i++)
	{
		if(temp_buffer[i] == 0x00) 
		{
			if(temp_buffer[i+1] == 0x00 && temp_buffer[i+2] == 0x00 && temp_buffer[i+3] == 0x01)
			{
				if((0 < i) && (i <= 21)) continue;
				next_bit = i; 
				printf("%d's bit : %d\n", count, next_bit);
				count++;
