...
typedef struct {
	int magic;	//매직넘버
	int hMFC;	//mfc 드라이버 파일 디스크립터
	int width;	//가로
	int height;	//세로
	int sizeStrmBuf;	//ouputbuffersize의 값 저장
	mfc_frame_buf_arg_t sizeFrmBuf;
	int displayStatus;
	int inter_buff_status;	//내부 버퍼의 상태?
	unsigned int virFreeStrmAddr;
	unsigned int phyStrmBuf;	//phyStrmBuf의 메모리 주소를 int로 지정
	unsigned int virStrmBuf;	//virStrmBuf의 메모리 주소를 int로 지정
	unsigned int virMvRefYC;
	mfc_frame_buf_arg_t phyFrmBuf;
	mfc_frame_buf_arg_t virFrmBuf;
	unsigned int mapped_addr;	//맵핑된 메모리 주소를 int로 지정
	mfc_common_args MfcArg;
	SSBSIP_MFC_CODEC_TYPE codec_type;	//코덱 타입
	SSBSIP_MFC_DEC_OUTPUT_INFO decOutInfo;
	unsigned int encodedHeaderSize;
	int encodedDataSize;	//인코딩 된 데이터의 사이즈가 저장 됨
	unsigned int encodedframeType;	//인코딩 된 프레임의 타입이 저장 됨
	int in_frametag;
	int out_frametag_top;	//인코딩 된 결과가 저장 됨
	int out_frametag_bottom;	//인코딩 된 결과가 저장 됨
	unsigned int encoded_Y_paddr;	//인코딩 된 y 영역이 저장 됨
	unsigned int encoded_C_paddr;	//인코딩 된 c 영역이 저장 됨
} _MFCLIB;
